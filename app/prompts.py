...
- If a note expresses minimum_battery_reserve as a PERCENTAGE of battery capacity (e.g. "keep at least 50% of
  the battery capacity"), you are given the battery's capacity_kwh below. Convert the percentage into an
  absolute kWh value yourself: minimum_energy_kwh = percentage * capacity_kwh. Never leave a percentage
  unconverted and never output a raw percentage number as minimum_energy_kwh.
...
def build_user_prompt(notes: list[str], battery_capacity_kwh: float) -> str:
    ...
    return (
        ...
        f"(Use this only to convert percentage-of-capacity reserve language into absolute kWh.)\n\n"
